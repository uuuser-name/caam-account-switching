...
func TestTagCommands_Extended(t *testing.T) {
	...
	defer func() {
		...
		if err := tagListCmd.Flags().Set("json", "false"); err != nil {
			t.Errorf("reset --json: %v", err)
		}
	}()
	...
	require.NoError(t, err)
	require.NoError(t, prof2.AddTag("home"))
	require.NoError(t, prof2.Save())
	...
	require.NoError(t, tagListCmd.Flags().Set("json", "true"))
	output, err = captureStdout(t, func() error {
