...
func (m *MockProvider) DisplayName() string {
	...
	return strings.ToUpper(m.id[:1]) + m.id[1:]
}
...
func (m *MockProvider) ImportAuth(ctx context.Context, sourcePath string, targetProfile *profile.Profile) ([]string, error) {
	...
	for _, f := range m.mockImportFiles {
		...
		if err := os.WriteFile(fullPath, []byte("mock-content"), 0o600); err != nil {
			return nil, err
		}
	}
...
func TestAuthCommands_Extended(t *testing.T) {
	...
	defer func() {
		...
		if err := authDetectCmd.Flags().Set("json", "false"); err != nil {
			t.Errorf("reset authDetect --json: %v", err)
		}
		if err := authImportCmd.Flags().Set("json", "false"); err != nil {
			t.Errorf("reset authImport --json: %v", err)
		}
		if err := authImportCmd.Flags().Set("name", ""); err != nil {
			t.Errorf("reset authImport --name: %v", err)
		}
		if err := authImportCmd.Flags().Set("source", ""); err != nil {
			t.Errorf("reset authImport --source: %v", err)
		}
	}()
	...
	require.NoError(t, authDetectCmd.Flags().Set("json", "true"))
	...
	require.NoError(t, authImportCmd.Flags().Set("json", "true"))
	require.NoError(t, authImportCmd.Flags().Set("name", "work"))
	...
	require.NoError(t, authImportCmd.Flags().Set("name", "mixed"))
	_, err = captureStdout(t, func() error {
		return authImportCmd.RunE(authImportCmd, []string{"ClAuDe"})
	})
	require.NoError(t, err)
	...
	require.NoError(t, authImportCmd.Flags().Set("name", "custom"))
	require.NoError(t, authImportCmd.Flags().Set("source", "/home/user/custom.json"))
	...
	require.NoError(t, authImportCmd.Flags().Set("source", tmpSource))
